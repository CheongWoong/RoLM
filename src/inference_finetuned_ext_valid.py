...
if __name__ == "__main__":
    ...
    if args.majority_voting:
        output_path = os.path.join(output_dir, f"{prompting_strategy}_finetuned_ext_majority_voting_raw_predictions_validation.jsonl")
    else:
        output_path = os.path.join(output_dir, f"{prompting_strategy}_finetuned_ext_raw_predictions_validation.jsonl")
    ...
    with jsonlines.open(input_path) as fin, open(output_path, "w") as fout:
        ...
        for idx, example in enumerate(tqdm(fin.iter())):
            ...
            for format_type in [str(num) for num in range(NUM_FORMATS)]:
                # Format example
                formatted_example = format_example_ext(example, dataset_name, prompting_strategy, format_num=int(format_type))
            ...
            # Save a formatted input example
            if idx == 0:
                with open(os.path.join(output_dir, f"{prompting_strategy}_ext_input_examples_validation.jsonl"), "w") as fout_input_examples:
