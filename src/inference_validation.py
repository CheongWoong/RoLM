...
if __name__ == "__main__":
    ...
    if args.majority_voting:
        output_path = os.path.join(output_dir, f"{prompting_strategy}_majority_voting_raw_predictions_validation.jsonl")
    else:
        output_path = os.path.join(output_dir, f"{prompting_strategy}_raw_predictions_validation.jsonl")
    ...
    with jsonlines.open(input_path) as fin, open(output_path, "w") as fout:
        ...
        for idx, example in tqdm(enumerate(fin.iter())):
            ...
            # Save a formatted input example
            if idx == 0:
                with open(os.path.join(output_dir, f"{prompting_strategy}_input_examples_validation.jsonl"), "w") as fout_input_examples:
