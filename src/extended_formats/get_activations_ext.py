...
from src.utils.prompt_formatting import format_example_ext
...
if __name__ == "__main__":
    ...
    layer_wise_path = os.path.join(output_dir, f"{prompting_strategy}_ext_layer_wise_hidden_states.dat")
    head_wise_path = os.path.join(output_dir, f"{prompting_strategy}_ext_head_wise_hidden_states.dat")
    ...
    with jsonlines.open(input_path) as fin:
        ...
        for idx, example in tqdm(enumerate(fin.iter())):
            for format_num in [num for num in range(pow(2, NUM_FORMATS_ELEMENTS))]:
                # Format example
                formatted_example = format_example_ext(example, dataset_name, prompting_strategy, format_num=format_num)
                ...
                layer_wise_memmap[idx, int(format_num)] = layer_wise_hidden_states
                head_wise_memmap[idx, int(format_num)] = head_wise_hidden_states
