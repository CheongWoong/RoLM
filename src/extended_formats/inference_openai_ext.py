...
if __name__ == "__main__":
    ...
    if args.majority_voting:
        ...
        output_path = os.path.join(output_dir, f"{prompting_strategy}_ext_raw_predictions.jsonl")
    ...
    with jsonlines.open(input_path) as fin, open(output_path, "w") as fout:
        ...
        for idx, example in tqdm(enumerate(fin.iter())):
            ...
            # Save a formatted input example
            if idx == 0:
                with open(os.path.join(output_dir, f"{prompting_strategy}_ext_input_examples.jsonl"), "w") as fout_input_examples:
